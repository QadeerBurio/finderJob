...
const AddJob = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState(null); // Initialized as null
      ...
      <Modal visible={openSkillModal} transparent style={{ flex: 1 }}>
        <View style={styles.mainView}>
          <View style={styles.listingView}>
            ...
            <FlatList data={selectedCategory !== null ? profiles[selectedCategory].keywords : []} renderItem={({ item, index }) => {
              console.log(item);
              return (
                <TouchableOpacity style={styles.profileItem}
                  onPress={() => {
